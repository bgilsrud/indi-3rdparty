...
{
    gss3CCD->ISGetProperties(dev);
}

void ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int num)
{
    gss3CCD->ISNewSwitch(dev, name, states, names, num);
}

void ISNewText(const char *dev, const char *name, char *texts[], char *names[], int num)
{
    gss3CCD->ISNewText(dev, name, texts, names, num);
}

void ISNewNumber(const char *dev, const char *name, double values[], char *names[], int num)
{
    gss3CCD->ISNewNumber(dev, name, values, names, num);
}
...
SSG3CCD::SSG3CCD()
{
    ...
    setVersion(SSG3_VERSION_MAJOR, SSG3_VERSION_MINOR);
}
...
bool SSG3CCD::Connect()
{
    ...
    dsi = SSG3::DeviceFactory::getInstance(nullptr);
#ifdef __APPLE__
    if (!dsi)
    {
        sleep(2);
    ...
    if (!dsi)
    {
        /* The vendor and product ID for all SSG3's (I/II/III) are the same.
            * When the Cypress FX2 firmware hasn't been loaded, the PID will
            * be 0x0100. Once the fw is loaded, the PID becomes 0x0101.
            */
        LOG_INFO("Unable to find SSG3. Has the firmware been loaded?");
        return false;
    }
    ccd = dsi->getCcdChipName();
    if (ccd == "ICX254AL")
    {
        LOG_INFO("Found a SSG3 Pro!");
    }
    else if (ccd == "ICX429ALL")
    {
        LOG_INFO("Found a SSG3 Pro II!");
    }
    else if (ccd == "ICX429AKL")
    {
        LOG_INFO("Found a SSG3 Color II!");
    }
    else if (ccd == "ICX404AK")
    {
        LOG_INFO("Found a SSG3 Color!");
    }
    else if (ccd == "ICX285AL")
    {
        LOG_INFO("Found a SSG3 Pro III!");
    }
    else
    {
        LOGF_INFO("Found a SSG3 with an unknown CCD: %s", ccd.c_str());
    }
...
bool SSG3CCD::initProperties()
{
    ...
    /* Vdd on during exposure property (gs)
       Actually, Meade envisage leaves Vdd always on for SSG3 III during
       exposure. However, this results in a significant amount of amp glow.
       Vdd auto mode also seems not to work properly for the SSG3 III.
    ...
    IUFillSwitchVector(&VddExpSP, VddExpS, 2, getDeviceName(), "SSG3 III exposure", "", IMAGE_SETTINGS_TAB, IP_RW,
...
/*******************************************************************************
** Set binning (currently implemented only for SSG3 III) (gs)
*******************************************************************************/

bool SSG3CCD::UpdateCCDBin(int hor, int ver)
{
    ...
    {
        IDMessage(getDeviceName(), "Only 1x1 and 2x2 binning is supported by SSG3 III.");
        LOG_INFO("Only 1x1 and 2x2 binning is supported by SSG3 III.");
        return false;
    }
}
...
bool SSG3CCD::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    ...
    {
        /* Vdd on/off switch for SSG3 III exposure control (gs) */
